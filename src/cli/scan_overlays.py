"""CLI entry point for accessibility overlay scanner."""
...
def main():
    """Main CLI entry point for accessibility overlay scanning."""
    parser = argparse.ArgumentParser(
        description=(
            "Scan government website pages for accessibility overlay products. "
            "Detects known overlay vendors (e.g. AccessiBe, UserWay, AudioEye) "
            "by matching page HTML against a list of vendor-specific signatures. "
            "Produces an annotated TOON file and persists results to the metadata "
            "database. This scan is intended to run on-demand rather than on a "
            "schedule, since overlays are not expected to change frequently."
        )
    )
    parser.add_argument(
        "--country",
    ...
    try:
        if args.all:
            if max_runtime_seconds is not None:
                print(
                    f"Scanning all countries for accessibility overlays "
                    f"(max runtime: {args.max_runtime} minutes)..."
                )
            else:
                print("Scanning all countries for accessibility overlays...")
            ...
            for country_stats in all_stats:
                if "error" in country_stats:
                    ...
                    complete_flag = (
                        "" if country_stats.get("is_complete", True) else " (partial)"
                    )
                    print(
                        f"{country_stats['country_code']}{complete_flag}: "
                        f"{country_stats['urls_scanned']} scanned, "
                        f"{country_stats['urls_with_overlays']} with overlays"
                    )
        else:
            ...
            print(f"Scanning {country_code} for accessibility overlays...")
            stats = asyncio.run(
                job.scan_country(
                    country_code,
                    toon_file,
            ...
            print(f"URLs with overlays:      {stats['urls_with_overlays']}")
            print(f"Total detections:        {stats['total_overlay_detections']}")
            if stats["vendor_counts"]:
                print("Overlay vendors found:")
                for vendor, cnt in sorted(
                    stats["vendor_counts"].items(), key=lambda x: -x[1]
                ):
                    print(f"  {vendor}: {cnt}")
            else:
                print("No overlay vendors detected.")
